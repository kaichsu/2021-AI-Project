...
from spirecomm.ai.route_ai import *
...
class SimpleAgent:
    ...
    def generate_map_route(self):
        ...
        # def generate_map_route(self):
        #     node_rewards = self.priorities.MAP_NODE_PRIORITIES.get(self.game.act)
        #     best_rewards = {self.game.screen.next_nodes[0].y: {node.x: node_rewards[node.symbol] for node in self.game.map.nodes[0].values()}}
        #     best_parents = {self.game.screen.next_nodes[0].y: {node.x: 0 for node in self.game.map.nodes[0].values()}}
        #     min_reward = min(node_rewards.values())
        #     map_height = max(self.game.map.nodes.keys())
        #     for y in range(self.game.screen.next_nodes[0].y, map_height):
        #         best_rewards[y+1] = {node.x: min_reward * 20 for node in self.game.map.nodes[y+1].values()}
        #         best_parents[y+1] = {node.x: -1 for node in self.game.map.nodes[y+1].values()}
        #         for x in best_rewards[y]:
        #             node = self.game.map.get_node(x, y)
        ...
        #     best_path = [0] * (map_height + 1)
        #     best_path[map_height] = max(best_rewards[map_height].keys(), key=lambda x: best_rewards[map_height][x])
        #     for y in range(map_height, self.game.screen.next_nodes[0].y, -1):
        #         best_path[y - 1] = best_parents[y][best_path[y]]
        #     self.map_route = best_path
    ...
    def get_current_state(self):
        ...
        for card in self.game.deck:
            upgrade += len(re.findall(r'\+', card.name))
        for card in self.game.deck:
            if card.name in BASE_GAME_CURSES:
                curse += 1
        ...
        state = dict()
        state['cards'] = len(self.game.deck) - upgrade - curse
        state['upgrade_cards'] = upgrade
        state['curse_cards'] = curse
        state['relics'] = len(self.game.relics)
        if self.game.ascension_level:
            state['ascension'] = self.game.ascension_level
        else:
            state['ascension'] = 0
        state['floor'] = self.game.floor
        state['potions'] = potion_num
        state['max_hp'] = self.game.max_hp
        state['current_hp'] = self.game.current_hp
        state['gold'] = self.game.gold

        return state


    def choose_best_route(self, node_rewards, x, y):
        node = self.game.map.get_node(x, y)
        mx = 0
        if node:
            for next_node in node.children:
                mx = max(self.choose_best_route(node_rewards, next_node.x, next_node.y), mx)
        return mx + node_rewards[node.symbol]
