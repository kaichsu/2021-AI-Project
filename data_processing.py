...
def process_runs(data_dir):
    ...
    tmp_dir = os.path.join('test_out', str(round(time.time())))
    os.mkdir(tmp_dir)
...
def process_run(data):
    ...
    potions_use_by_floor = {floor:data['potions_floor_usage'].count(floor) for floor in data['potions_floor_usage']}
    relics_by_floor = get_relics_by_floor(data)
    potions_by_floor = get_potions_by_floor(data)
    ...
    for floor in range(0, data['floor_reached']+1):
        ...
        if floor in potions_use_by_floor:
            process_use_potions(potions_use_by_floor[floor], current_potions, floor, unknowns)

        if floor in purchases_by_floor:
            try_process_data(partial(process_purchases, purchases_by_floor[floor], current_deck, current_relics, current_potions, data['relics'], floor, unknowns), floor, current_deck, current_relics, data, unknowns)
        ...
        if floor in potions_by_floor:
            process_potions(potions_by_floor[floor], current_potions, current_relics, floor, unknowns)
        ...
        # floor_data = process_floor(data, potion_use_by_floor, current_deck, current_relics, floor)
        floor_data = process_floor(data, current_deck, current_relics, current_potions, floor)
        processed_fights.append(floor_data)

    for floor in range(len(processed_fights)):
        if floor != len(processed_fights)-1:
            processed_fights[floor]['value'] = get_value(processed_fights[floor]['current_hp'], processed_fights[floor+1])
        else:
            processed_fights[floor]['value'] = get_value(100, processed_fights[floor])

        upgrade = 0
        curse = 0

        for card in processed_fights[floor]['cards']:
            upgrade += len(re.findall(r'\+', card))
        for card in processed_fights[floor]['cards']:
            if card in BASE_GAME_CURSES:
                curse += 1

        processed_fights[floor]['cards'] = len(processed_fights[floor]['cards']) - upgrade - curse
        processed_fights[floor]['upgrade_cards'] = upgrade
        processed_fights[floor]['curse_cards'] = curse
        processed_fights[floor]['relics'] = len(processed_fights[floor]['relics'])
...
def get_value(start_hp, end_state):
    upgrade = 0
    curse = 0

    for card in end_state['cards']:
        upgrade += len(re.findall(r'\+', card))
    ...
    return (math.atan(3*(end_state['current_hp']/start_hp) - 1.5) + 1)*(end_state['gold'] + end_state['max_hp'] + end_state['current_hp'] +
        80*(len(end_state['cards']) - upgrade) + 100*upgrade - 100*curse + 250*len(end_state['relics']) + 80*end_state['potions'])
...
# def process_floor(master_data, potion_use_by_floor, current_deck, current_relics, floor):
def process_floor(master_data, current_deck, current_relics, current_potions, floor):

    floor_data = dict()
    floor_data['cards'] = list(current_deck)
    floor_data['relics'] = list(current_relics)
    floor_data['ascension'] = master_data['ascension_level']
    floor_data['character'] = master_data['character_chosen']
    floor_data['floor'] = floor
    floor_data['potions'] = sum(current_potions)

    try:
        floor_data['path'] = master_data['path_per_floor'][floor]
    except Exception as e:
        floor_data['path'] = None

    if floor:
        floor_data['max_hp'] = master_data['max_hp_per_floor'][floor - 1]
        floor_data['current_hp'] = master_data['current_hp_per_floor'][floor - 1]
        floor_data['gold'] = master_data['gold_per_floor'][floor - 1]
        # floor_data['potions'] = floor in potion_use_by_floor
    else:
        floor_data['max_hp'] = master_data['max_hp_per_floor'][0]
        floor_data['current_hp'] = master_data['max_hp_per_floor'][0]
        floor_data['gold'] = 99
        # floor_data['potions'] = floor in potion_use_by_floor
    return floor_data


def process_use_potions(potions_use_by_floor, current_potions, floor, unknowns):
    current_potions.append(-potions_use_by_floor)
...
def process_potions(potions_by_floor, current_potions, current_relics, floor, unknowns):
    current_potions.append(1)
    if 'Potion Belt' in current_relics and sum(current_potions) > 5:
        current_potions = [1,1,1,1,1]
    elif sum(current_potions) > 3:
        current_potions = [1,1,1]
...
def process_purchases(purchase_data, current_deck, current_relics, current_potions, master_relics, floor, unknowns):
    ...
    purchased_potions = [1 for x in purchase_data if x not in purchased_cards and x not in purchased_relics]
    current_deck.extend(purchased_cards)
    current_potions.extend(purchased_potions)
...
def get_potions_by_floor(data):
    potions_by_floor = get_stats_by_floor_with_list(data, 'potions_obtained')
